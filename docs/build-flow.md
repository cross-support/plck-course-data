...
> **注意（リネームについて）**:
> - 元 PNG（講座セットフォルダ側）は `スライド{M}.png` の日本語名、plck-main 入力側は `{M}.png`（番号のみ）。**ファイル名は変わるが、バイナリ内容は完全一致が必須**（preflight が SHA-256 で照合）。
> - preflight は元ファイル名末尾の数字を抽出して plck 側 `{N}.png` と突き合わせる仕組みなので、**末尾番号さえ正しければ日本語接頭辞「スライド」は省略可**。
> - `cp -p` を使えばメタ情報（mtime 等）も保持されるが、ハッシュ照合に影響するのは中身のバイト列のみ。
