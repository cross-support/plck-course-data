...
PLCK 翻訳講座（ベトナム語 / 中国語 / 他言語）を LMS（クロスラーニング LMS）へ搭載するためのビルド絶対ルール。
...
3. 24本ZIP生成（`.gitkeep`/`.DS_Store`/`__MACOSX`を除外、`zip -X` で extra attr排除）
4. `verify_zip.sh` — 全ZIPの構造と参照整合性を検証（失敗で停止）

部分実行:
```bash
./tools/scripts/preflight.sh                        # 全24UNIT検査
...
_最終更新: 2026-04-21（中国語講座字形崩れ事故を受けて初版作成）_
