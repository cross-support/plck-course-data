...
2. **配信画像は JPEG(quality=80) 必須。ただし品質保証の正本は PNG。**
   ...
   - 配信画像は正本 PNG から `tools/scripts/png_to_jpg.sh`（内部で `magick in.png -quality 80 -strip out.jpg`）で生成した `{N}.jpg`。config の `img:` は `.jpg` を参照し、dist/ZIP には JPEG のみが含まれる
